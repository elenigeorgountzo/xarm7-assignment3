...
def forward_kinematics_RR(theta1, theta2):
    ...
    [0, 0, 1]])
    H_3_0 = np.array([
    [np.cos(theta1 + theta2), -np.sin(theta1 + theta2), l1*np.cos(theta1) + l2*np.cos(theta1 + theta2)],
    [np.sin(theta1 + theta2),  np.cos(theta1 + theta2), l1*np.sin(theta1) + l2*np.sin(theta1 + theta2)],
    [0, 0, 1]])
    # ============ END STUDENT SECTION ==============
